...
class UserModelTest(TestCase):
    ...
    def test_create_teacher(self):
        user = User.objects.create_user(username='teacher', email='t@test.com', password='123', role='teacher')
        self.assertEqual(user.role, 'teacher')
        self.assertTrue(user.is_teacher)

    def test_create_admin(self):
        user = User.objects.create_user(username='admin', email='a@test.com', password='123', role='admin')
        self.assertEqual(user.role, 'admin')
        self.assertTrue(user.is_admin)

    def test_user_str(self):
        user = User.objects.create_user(username='john', email='j@test.com', password='123', first_name='John')
