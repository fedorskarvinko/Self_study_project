...
class CourseModelTest(TestCase):
    ...
    def test_create_course(self):
        course = Course.objects.create(title='Python', description='Learn Python', owner=self.teacher)
...
class LessonModelTest(TestCase):
    ...
    def test_create_lesson(self):
        lesson = Lesson.objects.create(title='Lesson 1', content='Content', course=self.course, order=1)
...
class UserTestResultTest(TestCase):
    ...
    def test_result_passed(self):
        result = UserTestResult.objects.create(
            user=self.student, test=self.test, score=80,
    ...
    def test_result_failed(self):
        result = UserTestResult.objects.create(
            user=self.student, test=self.test, score=60,
            correct_answers=6, total_questions=10
        )
...
class CourseAPITest(TestCase):
    ...
    def test_student_cannot_create_course(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {token}')
        response = self.client.post('/api/courses/', {
            'title': 'Python', 'description': 'Learn Python'
        }, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_list_courses_as_student(self):
        Course.objects.create(title='Published', description='D', owner=self.teacher, is_published=True)
        Course.objects.create(title='Hidden', description='D', owner=self.teacher, is_published=False)
    ...
    def test_get_course_detail(self):
        course = Course.objects.create(title='Python', description='Desc', owner=self.teacher, is_published=True)
        token = self._get_token('student', '123')
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {token}')
...
class TestAPITest(TestCase):
    def setUp(self):
        ...
        self.question = Question.objects.create(test=self.test, text='2+2=?', question_type='single')
    ...
    def test_add_question(self):
        ...
        response = self.client.post(f'/api/tests/{self.test.id}/add_question/', {
            'text': 'New question?',
    ...
    def test_result_created(self):
        result = UserTestResult.objects.create(
            user=self.student, test=self.test,
            score=100, correct_answers=1, total_questions=1
        )
        self.assertTrue(result.is_passed)